This project is a simulation of a backend for a Battleship game where  class called ShipGame that allows two people to play the game Battleship (https://en.wikipedia.org/wiki/Battleship_(game)). Each player has their own 10x10 grid they place their ships on. On their turn, they can fire a torpedo at a square on the enemy's grid. Player 'first' gets the first turn to fire a torpedo, after which players alternate firing torpedos. A ship is sunk when all of its squares have been hit. When a player sinks their opponent's final ship, they win.
...
As a simple example, the ShipGame class can be used as follows: 
```
game = ShipGame()
